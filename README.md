...
If you follow the examples shown in the [TTree class page](https://root.cern.ch/doc/master/classTTree.html), you can see that the leaflist argument should look like "dimuon\_mass["+str(len(massarray))+"]/D". This leads to the creation instance that you can see in the template of newTree.Branch("dimuon\_mass",massarray,"dimuon\_mass["+str(len(massarray))+"]").
